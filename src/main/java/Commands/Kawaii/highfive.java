...
public class highfive implements Command {
    @Override
        ...
        if (msg.length() < 1) {
            ...
            event.getTextChannel().sendMessage(
                    new EmbedBuilder().setColor(Color.PINK)
                            .setDescription("**" + msg + "**, you got a highfive by **" + event.getMessage().getAuthor().getName() + "!** :3")
                            .setImage("https://cdn.weeb.sh/images/S1kKq1XiZ.gif").build()).queue();
    }
}
