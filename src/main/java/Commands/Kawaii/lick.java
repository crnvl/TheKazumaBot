...
public class lick implements Command {
    @Override
        ...
        if (msg.length() < 1) {
            ...
            event.getTextChannel().sendMessage(
                    new EmbedBuilder().setColor(Color.PINK)
                            .setDescription("**" + msg + "**, you were licked by **" + event.getMessage().getAuthor().getName() + "!** :3")
                            .setImage("https://cdn.weeb.sh/images/rykRHmB6W.gif").build()).queue();
    }
}
