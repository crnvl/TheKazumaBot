...
public class nom implements Command {
    @Override
        ...
        if (msg.length() < 1) {
            ...
            event.getTextChannel().sendMessage(
                    new EmbedBuilder().setColor(Color.PINK)
                            .setDescription("**" + msg + "**, you got a nom! from **" + event.getMessage().getAuthor().getName() + "!** :wink:")
                            .setImage("https://cdn.weeb.sh/images/HkGGy1Yvb.gif").build()).queue();
    }
}
